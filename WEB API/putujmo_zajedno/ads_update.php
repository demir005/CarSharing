...
if(isset($_POST['tip_oglasa']) && isset($_POST['polazak']) 
 ...
 //    $password = 'admin';
    ...
    try{
    	...
    	$result = $con->prepare('UPDATE oglasi SET tip_oglasa=?, polazak=?, odrediste=?, datum=?, 
                                    vrijeme=?, br_mjesta=?, flex_dana=?, info=?, trosak=?, cijena=?, korisnik_id=? WHERE id = ?');
        ...
        $result->bindValue(12, $id, PDO::PARAM_INT); 
   ...
   // $result = $con->query("INSERT INTO korisnici(ime, prezime, email, password, telefon, fb) VALUES('$ime', '$prezime', '$email', '$pass', '$telefon, '$fb')");
	...
	if ($result) {
      	$poruka["uspjeh"] = 1;
        $poruka["tekst"] = "update successfully";
